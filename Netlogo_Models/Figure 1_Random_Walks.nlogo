...
to random_walk
  ask turtles [
   ...
   set current_y ycor
   update_RSD
   set color scale-color magenta RSD max-pxcor min-pxcor
   ifelse random 2 = 0 [rt min_angle + random (max_angle - min_angle)]
   [lt min_angle + random (max_angle - min_angle)]
   set step_len random-normal mu sigma
   fd step_len
...
to trace
  ifelse add_pen = true [pen-down] [pen-up]
end

to update_RSD
  let x_component (current_x - init_x)
  let y_component (current_y - init_y)
...
"" ""
PENS
"default" 1.0 1 -16777216 true "set-histogram-num-bars num_particles" "histogram [RSD] of turtles"
